...
First it's important to not that this circle is created by using 6 equal parts. So each "path" creates one section of the circle like so:
...
This draws a cubic Bézier curve from the current point to (x,y) using (x1,y1) as the control point at the beginning of the curve and (x2,y2) as the control point at the end of the curve.
